    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6">
      ...
      <div className="bg-white rounded-[2rem] sm:rounded-[3rem] shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="block md:hidden divide-y divide-gray-100">
          {loading ? (
            ...
            <div className="p-10 text-center text-gray-400 font-bold uppercase tracking-tighter text-xl">No Citizens Identified</div>
          ) : (
            users.map((user) => (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-4">
                    <input
                      ...
                      className="w-5 h-5 rounded-lg border-gray-200 text-blue-600 focus:ring-blue-500 shrink-0"
                    />
                    <div className="w-12 h-12 bg-gradient-to-br from-gray-100 to-gray-50 rounded-2xl flex items-center justify-center text-gray-400 font-black border border-gray-100 overflow-hidden shadow-sm shrink-0">
                      {user.avatar ? (
                    ...
                    <div>
                      <p className="font-black text-gray-900 text-sm uppercase tracking-tighter mb-0.5 line-clamp-1">{user.name}</p>
                      <p className="text-xs text-gray-400 font-medium lowercase truncate max-w-[150px]">{user.email}</p>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    <span className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest shadow-sm border ${user.role === 'Admin' ? 'bg-purple-50 text-purple-700 border-purple-100' :
                ...
                <div className="flex items-center justify-between pl-9 pr-1">
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50 rounded-lg border border-gray-100">
                      ...
                      <span className={`text-[9px] font-bold uppercase ${user.status === 'Active' ? 'text-emerald-700' : 'text-red-700'}`}>{user.status}</span>
                    </div>
                    <span className="text-xs font-black text-gray-900">{user.xp?.toLocaleString() || 0} XP</span>
                  </div>

                  <div className="flex gap-2">
                    <button onClick={() => handleShowReport(user)} className="p-2 bg-gray-50 text-gray-400 rounded-xl active:scale-95 transition-all">
                      <BarChart2 size={16} />
                    </button>
                    <button onClick={() => handleEditUser(user)} className="p-2 bg-blue-50 text-blue-600 rounded-xl active:scale-95 transition-all">
                      <Edit size={16} />
                    </button>
                    <button onClick={() => handleDeleteUser(user.id)} className="p-2 bg-red-50 text-red-600 rounded-xl active:scale-95 transition-all">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
      ...
      {showUserModal && (
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] sm:rounded-[3rem] shadow-2xl p-6 sm:p-10 w-full max-w-lg border border-gray-100 animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-8">
              <div>
                <h3 className="text-xl sm:text-2xl font-black uppercase tracking-tight text-gray-900">{editingUser ? 'Modify Citizenship' : 'Onboard New Citizen'}</h3>
            ...
            <form onSubmit={handleUpdateUser} className="space-y-6 sm:space-y-8">
              ...
              <div className="flex gap-4 pt-2">
                ...
                <button
                  type="submit"
                  className="flex-2 px-8 sm:px-10 py-4 bg-blue-600 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-xl shadow-blue-200 hover:bg-blue-700 transition-all"
                >
                  Deploy Changes
                </button>
              </div>
      ...
      {showReportModal && (
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] sm:rounded-[4rem] shadow-2xl p-6 sm:p-12 w-full max-w-4xl border border-gray-100 animate-in slide-in-from-bottom-10 duration-500 overflow-y-auto max-h-[90vh] relative">
            <button
              onClick={() => setShowReportModal(false)}
              className="absolute top-6 right-6 sm:top-8 sm:right-8 p-3 bg-gray-50 text-gray-400 hover:text-gray-900 rounded-2xl transition-all z-10"
            >
              <X size={24} />
            </button>
            ...
            <div ref={reportRef} className="bg-white">
              <div className="flex flex-col sm:flex-row justify-between items-start mb-8 sm:mb-12 gap-4">
                <div>
                  ...
                  <h3 className="text-3xl sm:text-4xl font-black text-gray-900 uppercase tracking-tighter leading-none mb-2">{editingUser?.name}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-10 mb-8 sm:mb-12">
                <div className="bg-gray-50 p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-gray-100">
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Core Competency</p>
                  <div className="flex items-end gap-2">
                    <span className="text-3xl sm:text-4xl font-black text-gray-900">{editingUser?.xp?.toLocaleString() || 0}</span>
                ...
                <div className="bg-gray-50 p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-gray-100">
                  ...
                  <div className="flex items-end gap-2">
                    <span className="text-3xl sm:text-4xl font-black text-gray-900">{editingUser?.completedSimulations || 0}</span>
                ...
                <div className="bg-gray-50 p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-gray-100">
                  ...
                  <div className="flex items-end gap-2">
                    <span className="text-3xl sm:text-4xl font-black text-gray-900">{Math.floor((editingUser?.activeSeconds || 0) / 3600)}</span>
                    <span className="text-sm font-black text-amber-600 mb-1 uppercase">Hours</span>
                  </div>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10 h-auto sm:h-[300px]">
                <div className="bg-gray-50 rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-8 border border-gray-100 flex flex-col items-center justify-center relative overflow-hidden min-h-[300px]">
                  ...
                  <div className="w-full h-full flex items-center justify-center mt-4">
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart cx="50%" cy="50%" outerRadius="80%" data={[
                        { subject: 'Audit', A: editingUser?.auditCorrect || 0, fullMark: editingUser?.auditTotal || 100 },
                        { subject: 'Tax', A: editingUser?.taxCorrect || 0, fullMark: editingUser?.taxTotal || 100 },
                        { subject: 'Law', A: editingUser?.caselawCorrect || 0, fullMark: editingUser?.caselawTotal || 100 },
                        { subject: 'Rules', A: editingUser?.correctAnswers || 0, fullMark: editingUser?.totalQuestions || 100 },
                ...
                <div className="bg-gray-50 rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-8 border border-gray-100 flex flex-col min-h-[300px]">
            ...
            <div className="mt-8 sm:mt-12 flex gap-4">
              <button
                ...
                className="flex-1 py-4 sm:py-5 bg-gray-900 text-white rounded-[2rem] font-black uppercase tracking-widest text-xs shadow-xl shadow-gray-200 active:scale-95 transition-all flex items-center justify-center gap-3"
              >
                <Download size={18} />
