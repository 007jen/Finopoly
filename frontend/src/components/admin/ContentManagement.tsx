  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6">
      ...
      <div className="flex flex-wrap gap-3">
        {(['caselaw', 'audit', 'tax', 'arcade'] as ContentType[]).map((tab) => (
          <button
            key={tab}
...
const ContentForm: React.FC<ContentFormProps> = ({ contentType, item, onSave, onCancel }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-300">
      <div className="bg-white rounded-[3.5rem] shadow-2xl p-12 w-full max-w-4xl border border-gray-100 animate-in zoom-in-95 duration-300 overflow-y-auto max-h-[90vh]">
        <div className="flex justify-between items-start mb-10">
          <div>
            ...
            <h3 className="text-3xl font-black text-gray-900 uppercase tracking-tighter leading-none">{item ? 'Recalibrate' : 'Manifest'} Resource</h3>
            <p className="text-gray-400 font-bold text-sm mt-2">Configure logic protocols and yield parameters.</p>
          </div>
          <button
            onClick={onCancel}
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="flex gap-6 pt-6">
            <button
              type="button"
              onClick={onCancel}
              className="flex-1 py-5 bg-gray-100 text-gray-500 rounded-[2rem] font-black uppercase tracking-widest text-[10px] hover:bg-gray-200 transition-all active:scale-95"
            >
              Cancel Protocol
            </button>
            <button
              type="submit"
              className="flex-2 px-12 py-5 bg-blue-600 text-white rounded-[2rem] font-black uppercase tracking-widest text-[10px] shadow-2xl shadow-blue-200 hover:bg-blue-700 transition-all active:scale-95"
            >
              Authorize & Deploy
            </button>
          </div>
